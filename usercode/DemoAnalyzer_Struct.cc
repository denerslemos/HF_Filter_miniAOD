...
#include "DataFormats/HeavyIonEvent/interface/HFFilterStruct.h" //this line is needed to access the HF Filters
//
// class declaration
//
...
class DemoAnalyzer : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
   ...
   private:
      ...
      // ----------member data ---------------------------
      edm::EDGetTokenT<pat::HFFilterStruct> filters_;
        int ev=0;
...
DemoAnalyzer::DemoAnalyzer(const edm::ParameterSet& iConfig)
 :
  filters_(consumes<pat::HFFilterStruct>(iConfig.getParameter<edm::InputTag>("filters")))
{
...
DemoAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    edm::Handle<pat::HFFilterStruct> filt;
    iEvent.getByToken(filters_, filt);

    ev=ev+1;
    if(filt->numMinHFTowers4<2){evfail=evfail+1;}else{evpass=evpass+1;}
